...
主に背景の描画処理に注力し，素材の作成などを行った．また，week22のBGMなどの処理を一部見直した．

### SdlSystemの処理

Mixerの構築部分が責務過多になっており，Mix_Init()とMix_OpenAudio()を同時に実行していたため，一部のみ初期化されたままプログラムが終了する可能性があった．
そのため，Mix_Init()とMix_OpenAudio()でそれぞれコンストラクタを用意した．
...
### BackgroundRendererの実装

背景を描画する専用のクラスBackgroundRendererを作成した．肝となったのはレイヤの概念であり，単純な背景画像１枚を描画するのではなく，画家のアルゴリズムを用いて重ね合わせで背景を構築した．

重要な点として，背景のスクロール処理がある．カメラが導入されており，プレイヤーが画面右へ進むと，背景もスクロールする．このとき，背景がスクリーンぴったりではスクロール開始と同時に画像が途切れてしまう．このスクロール開始時に背景が途切れないように，画面外に視差効果parallaxFactorを加味してスクリーンよりも少し先を描画している．

```cpp
SDL_Rect dst1{-scroll_X, 0, screen_W, screen_H};
```

また描画範囲がスクリーンを一周するようにしている．

```cpp
const int scroll_X = static_cast<int>(camera.x * layer.parallaxFactor) % screen_W;
```

また，視差効果として[0.0, 1.0]の範囲の係数を導入している．これによって，スクロールに若干の差異が生まれる．画家のアルゴリズムと視差効果によるスクロールで，遠景はゆっくり，近景は若干早くスクロールするようになった．
...
描画は奥から前と実施することである．単純ではあるが，手前のオブジェクトが奥のオブジェクトを隠すという見えない部分の描写ができる．奥から前へ全ての画像を描画するため，コストは高い．しかしながら，今回の2Dマリオライクゲームでは背景はあくまでも演出であり，現状複雑な描画ではないため許容できると判断した．

なお，交差する背景などの描画は画家のアルゴリズムでは表現できないため，背景をステージに組み込むなら別の重なりを表現できる手法が必要となる．

### 背景の読み込み

背景はstage.jsonで指定できるようにしている．これによって，背景を切替えるだけの修正でソースを触る必要がなくなった．

BackgroundIdを導入し，背景をIDで指定できるようにした．続いてこれまでのrenderAssetContextに組み込み，StageDefinitionLoaderで読み込む流れに組み込んだ．

### 背景装飾

背景において，雲や星など多様な配置を取りたい場合がある．この場合，背景画像を編集するより，小さいpng画像を配置するようなイメージで設定できる方が利便性が高いと判断し，背景装飾をBackgroundRendererに組み込んだ．

基本的には背景の描画処理を踏襲しているが，画面外に出たとき，ループではなく消えるようにしている．これによって演出が柔軟にできる一方，長いステージでは設定が煩雑になるという欠点もある．

場合によっては画面外にでたらループさせる，といった処理も組み込むことを検討する．

### week23の課題

- 背景装飾の追加
  - StarやSunなどを追加したい
- 背景レイヤの構成がPlayingScene::loadBackground()にかかれている
  - stage.jsonなどに定義できれば修正が容易となる

week24では画面シェイク処理を実装予定である．
...
その他，png画像なのでファイル最適化用のコマンドラインロスレスツールのpngcrushも有効であることを確認している：
...
なお，元ファイルのバックアップは必ず取っておいたほうが良い．

## week24

### week24の概要

week23で課題となっていた背景構築処理のリファクタリングを行い，PlayingSceneの肥大化を解消した．
また，ゲームに演出として組み込める画面シェイクを仮実装した．

### 背景構築処理のリファクタリング

week23時点では，PlayingScene::loadBackground()内で背景のレイヤ構成(どのテクスチャを使うか，parallaxFactorをいくつにするか)をswitchで直接記述していた．
ステージ数の増加に伴い，この関数が肥大化することを避けるため，背景に関する知識をPlayingSceneの外へ切り出した．

#### BackgroundDefinition.hpp

背景IDに対応するレイヤ構成のデータ定義をBackgroundDefinition名前空間に集約した．

getLayerDefs()は従来std::vectorを返す実装を検討したが，呼び出しのたびにヒープ確保が走るためコストが高い．
そこで，static constexpr配列に対するビューとしてstd::span\<const BackgroundLayerDef\>を返す方式に変更した．
static constexpr配列はコンパイル時に確定し，バイナリのデータセグメントに配置されるため，getLayerDefs()の呼び出しコストはポインタを返すのみとなる．
...
#### BackgroundPresetBuilder

BackgroundRendererのレイヤ/装飾登録処理をBackgroundPresetBuilder名前空間に分離した．
テクスチャ解決ヘルパ(selectLayerTexture / selectDecorationTexture)はcppの無名名前空間に置き，外部へ公開しない設計とした．

```cpp
// BackgroundPresetBuilder.cpp (無名namespace内)
namespace{
...
void BackgroundPresetBuilder::build(
    ...
    for(const auto& ld : BackgroundDefinition::getLayerDefs(stageDef.backgroundId)){
        bgRenderer.addLayer(selectLayerTexture(textures, ld.texId), ld.parallaxFactor);
    }
    for(const auto& bd : stageDef.bgDecorations){
        bgRenderer.addDecoration(
            selectDecorationTexture(textures, bd.type),
            bd.world_X, bd.screen_Y, bd.parallaxFactor
        );
    }
}
```

この分離により，PlayingScene::loadBackground()は1行のAPIの呼び出しのみになる．
...
新しい背景IDを追加する際の変更箇所はBackgroundId.hpp・BackgroundDefinition.hpp・BackgroundPresetBuilder.cppの3箇所に限定され，PlayingSceneを触らずに済む設計となった．

### 画面シェイクの仮実装

ボス登場などの演出として画面シェイクを仮実装した．現状はPlayingScene内に一時的に実装しており，今後専用システムへ切り出す予定である．

#### 設計の方針

シェイクはカメラそのものを書き換えず，render()内でコピーしたCamera shakenにオフセットを加算して描画に使用する．これにより，ゲームロジック側のctx.cameraはシェイクの影響を受けない．ゲーム上のオブジェクトであるプレイヤーなどはシェイクの影響を受けるが，スコアなどの文字列の表示はシェイクの影響を受けないように，これまで通りctx.cameraを使用する．
...
- startCameraShake(duration, magnitude)：シェイクの持続時間と強さを設定する
- updateCameraShake(delta)：update()内で毎フレーム呼び，時間経過で振幅を減衰させる

```cpp
void PlayingScene::startCameraShake(double duration, double magnitude){
    // 複数のシェイクが重なった場合は大きい値を採用する
...
void PlayingScene::updateCameraShake(double delta){
    if(shakeTimer <= 0.0){
        shakeOffset_X = shakeOffset_Y = 0.0;
        return;
    }
    ...
    shakeOffset_Y = shakeDist(ctx.randomCtx.random) * currentMag * 0.5; // 縦揺れは控えめ
}
```

ランダムな揺れにはstd::uniform_real_distribution<double>を使用している．現状はPLayingSceneに直接画面シェイクAPIを記述しているため，演出として組み込めるのがボス戦トリガー発火時(updateBossBattleTrigger())のみであった．そのためひとまずstartCameraShake(0.30, 12.0)を呼び出し画面シェイクをテストで実装している．

### week24の課題

- 画面シェイクの切り出し
  - 現状はPlayingScene内に仮実装しており，IGameEventsなどを介してCollisionSystemなどのサブシステムからもシェイクを起動できるように設計を見直す必要がある
  - プレイヤーと敵の接触・敵弾ヒット時などにもシェイクAPIを呼び，演出の質を向上させる
...
week26で区切りをつけたPlayingSceneの演出強化に続き，week27ではゲーム全体のシーンを整備した．
主な実装内容は，TitleScene・GameOverScene・ClearScene・ResultSceneへの背景描画の導入，各シーンのUI見直し，ボスAIの行動パターンの強化，およびバグ修正である．

### PlayingScene以外のシーン背景の導入

#### 設計の背景

week26以前，背景描画はPlayingSceneのみに実装されていた．
PlayingSceneはBackgroundRendererとCameraを直接保持して背景を描画していたが，これをそのままTitleSceneなどへ持ち込むと，固定カメラの設定やBackgroundPresetBuilderの呼び出しが各シーンに散在することになる．
そこで，非プレイシーン向けに背景描画をまとめたSimpleSceneBackgroundクラスを新設した．

#### SimpleSceneBackgroundクラス

BackgroundRendererと固定カメラ（画面全体を映す）をラップする小さなクラスである：

```cpp
class SimpleSceneBackground{
private:
    BackgroundRenderer bgRenderer;
    Camera fixedCamera;     // (0, 0, WINDOW_WIDTH, WINDOW_HEIGHT)
public:
...
コンストラクタはBackgroundRendererとCameraを初期化子リストで初期化する：
...
利用側はonEnter()でsetPreset()，onExit()でclear()を呼ぶだけである：

```cpp
void TitleScene::onEnter(){
    background.setPreset(ctx.renderAssets.bgTextures, BackgroundId::Forest);
}
...
#### BackgroundPresetBuilderのオーバーロード追加

PlayingSceneはStageDefinitionを持つため，既存のbuild(BackgroundRenderer&, BackgroundTextureContext&, StageDefinition&)が使える．
しかし，非プレイシーンはStageDefinitionを持たない．そこでBackgroundIdを直接受け取るオーバーロードを追加した：
...
void BackgroundPresetBuilder::build(
    ...
    bgRenderer.clearLayers();
    bgRenderer.clearDecoration();
    for(const auto& def : BackgroundDefinition::getLayerDefs(id)){
        bgRenderer.addLayer(selectLayerTexture(textures, def.texId), def.parallaxFactor);
    }
    // 装飾は使わない
}
```

StageDefinition版との違いは，装飾（雲・星など）を追加しない点だけである．

#### 各シーンへの適用

| シーン | 背景ID |
| --- | --- |
| TitleScene | BackgroundId::Forest |
| GameOverScene | BackgroundId::darkBg（DarkSky単色） |
| ClearScene | BackgroundId::lightBg（Sky単色） |
| ResultScene | BackgroundId::lightBg（Sky単色） |

BackgroundId::lightBgとdarkBgはForest/DarkForest/HellForestとは異なり，単色の空レイヤ1枚で構成されるシンプルな背景である：

```cpp
static constexpr BackgroundLayerDef lightBg[] = {
    {BgLayerTextureId::Sky, 0.0},   // parallaxFactor=0.0 → カメラ移動で動かない
};
...
parallaxFactorが0.0であり，固定カメラと組み合わせることで単色の静止背景として機能する．

### シーン演出の見直し

#### タイトルフォントの追加

TitleSceneのゲームタイトル表示に，既存の通常フォントより大きいフォントが必要となった．
TextRenderContextにlargeFontメンバを追加し，フォントの種類を増やせるよう拡張した：

```cpp
struct TextRenderContext{
    Text& font;         // 通常フォント
    Text& largeFont;    // 大きめのフォント（タイトル・GameOverなどで使用）
    TextTexture& scoreText;
    TextTexture& fpsText;
};
```

TitleScene・GameOverScene・ClearScene・ResultSceneはいずれも見出しテキストにctx.textRenderCtx.largeFontを使用している．

#### TitleSceneのUI

TitleSceneには以下の演出・機能を追加した：

- ゲームタイトルのフェードイン（2秒かけてalpha 0→255に遷移）
- 「Press ENTER to Start」の点滅（0.5秒周期）
- 「Press ESC to Quit」の常時表示
- 操作方法テキストの画面下部表示

フェードインはdeltaを用いてtitleFadeを毎フレーム加算し，TextureのAlpha値へ反映する：

```cpp
titleFade += delta * 0.5;   // 2秒で1.0に到達
...
TitleSceneのESCキーによるゲーム終了は，シーン遷移イベントと同様のイベント駆動方式で実装した．
キーポーリングでゲームループのrunning変数を直接操作する方式と比較すると，イベント駆動方式はTitleSceneがゲーム全体のループを知らずに済むため疎結合である．

GameEventのvariantにRequestGameQuitEventを追加し，IGameEventsにrequestGameQuit()を追加した：
...
#### events.clear()によるイベント消失バグと修正

当初，RequestGameQuitEventの消費をprocessEvents()(フレーム開始時のSDLイベント処理関数)で行っていたため，ESCを押してもゲームが終了しない問題が発生した．

原因はGame::update()の処理順だった：

```text
currentScene->update(delta)  // TitleSceneがRequestGameQuitEventをバッファへ追加
consumeEvents(events)         // RequestGameQuitEventは未消費
events.clear()               // ← RequestGameQuitEventが消去される
// 次フレームのprocessEvents()で消費しようとしても既にイベントが消えている
```

修正は，Game::update()内でevents.clear()の直前にRequestGameQuitEventを消費することである：

```cpp
void Game::update(double delta){
    currentScene->update(delta);
    input->update();
    soundSystem->process(events);
    consumeEvents(events);
    // ゲーム終了イベントをclear()より前に消費する
    events.consumeIf(
        [](const GameEvent& ev){
            return std::holds_alternative<RequestGameQuitEvent>(ev);
        },
        [&](const GameEvent&){ running = false; }
    );
    events.clear();
...
### DropThroughブロックの水平衝突バグ修正

#### DropThroughブロックの問題の概要

BlockType::DropThrough(すり抜け床)をジャンプで下から上へ通過する際，キャラクタがブロックの横へはじき出される挙動が発生していた．

原因はresolveHorizontalBlockCollision()がDropThroughブロックを水平衝突の対象に含めていたことである．
ブロック内部にキャラクタが重なっている状態で「水平めり込み量 < 垂直めり込み量」が成立すると，水平方向の補正が発動する：

- overlapBottom = blockBottom - entityTop  → キャラが上昇するにつれて大きくなる
- horizontalOverlap < overlapBottom → 横はじき出し発生

#### DropThroughブロックの修正内容

resolveHorizontalBlockCollision()のブロック種別フィルタからDropThroughを除外した：

```cpp
// 修正前
if(b.type != BlockType::Standable
&& b.type != BlockType::DropThrough  // ← これを削除
&& b.type != BlockType::Question
...

// 修正後: DropThroughは水平方向に作用しない
if(b.type != BlockType::Standable
&& b.type != BlockType::Question
...
```

DropThroughは「上から乗れる・下からすり抜けられる一方通行床」であり，水平方向には壁として機能させないことで，通常の床と差別化することとした．

### ボスAIの強化

#### ボスAI強化の概要

week26のボスは固定砲台（位置を変えずにファイアボールを発射するだけ）だった．
week27では「プレイヤーへ接近しながらジャンプ」と「踏みつけによるダメージ」を追加し，ボス戦の戦略性を高めた．

#### 行動パターン

BossEnemy::think()を改修し，3つのフェーズで行動を制御する：

```text
1. 無敵時間中（被ダメ直後）: プレイヤーと反対方向へ逃げ + ジャンプ，攻撃しない
2. 追跡範囲外（|dx| > CHASE_RANGE）: 停止
3. 追跡範囲内: プレイヤー方向へ移動 + 定期的にジャンプ + 視界内なら射撃
```
...
void BossEnemy::think(double delta, const EnemySensor& es){
    ...
    if(invincibleTimer > 0.0){
        ...
        if(isOnGround()) setVerticalVelocity(-BossConfig::JUMP_VELOCITY);
        return;
    }
    // 追跡範囲外 → 停止
...
なお当初，素直にボス戦を実施すると，ボスが無敵時間であろうと踏み続けることで無傷でボスを倒すことができてしまった．
無敵時間(invincibleTimer > 0)を逃げ行動のトリガーとして流用することで，「被ダメ後に逃げる」挙動になった．

#### プレイヤーによる踏みつけの実装

ボスとプレイヤーの接触判定はBossBattleSystem::resolveBossPlayerCollision()として切り出した．
通常敵の踏みつけ処理(CollisionSystem::resolveEnemyCollision)と同じPlayerEnemyCollision::resolvePlayerEnemyCollision()ユーティリティを再利用している：

```cpp
void BossBattleSystem::resolveBossPlayerCollision(Player& player, IGameEvents& events){
    if(!state.isActive() || !boss.isAlive()) return;
    auto result = PlayerEnemyCollision::resolvePlayerEnemyCollision(
        player.getCollisionRect(),
        player.getPrevFeetCollision(), player.getFeetCollision(),
        player.getVerticalVelocity(), boss.getCollisionRect()
    );
...
SimpleSceneBackgroundによって非プレイシーンの背景実装が統一され，ボスAIの強化によってボス戦にある程度の駆け引きが生まれた．
これ以上ゲームの要素を追加することはせず，2Dアクションゲームを完成させるべく今後は調整を進めていく．
今後の予定はテストプレイによるステージ構成の調整，および技術・設計解説記事の執筆である．

## week28

### week28の概要

week27でゲーム全体のシーン・演出を整備し，ゲームの機能追加は一区切りとした．
week28では各ソースに対してGoogleTestを用いた単体テストを整備し，手動プレイテストへ移行するための品質確認を実施することとした．
また，ボス戦を整備したことでプライトが常に右を向いていた違和感に気がついたので，修正しておいた．

### ボスのスプライト方向修正

#### ボスのスプライト方向バグの概要

BossEnemy::think()はプレイヤーへの移動速度(hv)を更新していたが，描画方向を決めるdirメンバ変数の更新を行っていなかった．
Characterクラスのdraw()はdir == Direction::LeftをflipX引数としてRenderer::draw()へ渡す．
dirを更新する処理が抜けていたため，ボスのスプライトの方向が右で固定されていた．

```cpp
void Character::draw(Renderer& renderer, const Camera& camera){
    sprite.setPosition(static_cast<int>(x), static_cast<int>(y));
    renderer.draw(sprite, camera, dir == Direction::Right);  // dirが固定→常に右向き
}
```

#### ボスのスプライト方向バグの修正内容

BossEnemy::think()のクールダウン減衰直後に，プレイヤーの方向に基づいてdirを更新するよう1行追加した：
...
ゲームロジックに関わる主要コンポーネントを対象として，SDL描画やウィンドウ生成を必要としない「ロジック層」のテストをGoogleTest(gtest) + GoogleMock(gmock)で整備した．
各テストは正常系・異常系・境界値の3観点から構成した．

#### テスト対象一覧

| テストファイル | 対象クラス/関数 | 主なテスト観点 |
| --- | --- | --- |
| test_BackgroundDefinition | BackgroundDefinition::getLayerDefs() | 背景レイヤー数・texId・parallaxFactor・不明IDのthrow |
| test_CameraShakeController | CameraShakeController | シェイク有無・停止タイミング・不正入力・未start時の無効 |
| test_GameEventBuffer | GameEventBuffer | 全イベント型のエンキュー・consumeIf・clear・空バッファ |
| test_Physics_logic | Physics::resolve*() | 着地・天井衝突・水平衝突・DropThrough各種・Breakable/UsedQuestion |
| test_PlayerDeathController | PlayerDeathController | 点滅・演出終了タイミング・reset |
| test_StageDefinitionLoader | StageDefinitionLoader | JSON読み込み・不明背景ID throw・存在しないファイル throw |
| test_BlockLevelLoader | BlockLevelLoader | 正常データ読み込み・不正データのthrow |
| test_resolvePlayerEnemyCollision | PlayerEnemyCollision::resolve*() | 踏みつけ・側面衝突・非交差・境界値 |
| test_GameUtil | GameUtil::trim/intersects/blockToRect/rebuildBlockRects/isOutOfWorldBounds | 各ユーティリティ関数の正常系・境界値 |

#### CameraShakeControllerのテスト実装

CameraShakeControllerのupdate()はテンプレート関数であり，乱数エンジンを引数に取る：
...
テストでは乱数シードを固定したメルセンヌツイスタ(std::mt19937)を渡すことで，再現性を確保している：

```cpp
std::mt19937 rng{9999};  // シード固定で再現性を確保
shake.start(1.0, 10.0);
shake.update(0.016, rng);
const Camera shaken = shake.applyToCamera(camera);
EXPECT_TRUE(shaken.x != camera.x || shaken.y != camera.y);
```

#### GameEventBufferのconsumeIfテスト

consumeIf()は「指定したvariantのメンバのイベントのみ取り出して後ろを詰め順序を保つ」という動作をする．
このテストでは，addScore/playSound/addScore/spawnParticle/addScoreと混在したバッファからaddScoreだけを抽出し，スコア合計値と残りイベントの順序を両方確認した：

```cpp
events.consumeIf(
    [](const GameEvent& ev){ return std::holds_alternative<AddScoreEvent>(ev); },
    [&](const GameEvent& ev){ consumedScore += std::get<AddScoreEvent>(ev).delta; ++count; }
);
EXPECT_EQ(count, 3);
EXPECT_EQ(consumedScore, 25);           // 10 + 20 + (-5)
ASSERT_EQ(events.items().size(), 2u);   // playSoundとspawnParticleが残る
```

#### 2フレームにまたがる物理テスト

DropThroughブロックを「踏み抜いた後，同じブロックに再着地しないか」を確認するために，2フレーム分のVerticalCollisionStateを連続で渡すテストを作成した：

```cpp
// フレームN: ignoreDropThrough=trueで踏み抜き
Physics::resolveBlockCollisionFromTop(dropFrame, blocks);
EXPECT_FALSE(dropFrame.onGround);
EXPECT_GT(dropFrame.newFeet, floor.y);  // blockTopを越えた

// フレームN+1: prevFeet > blockTopになったので再着地しない
VerticalCollisionState nextFrame{ .prevFeet = dropFrame.newFeet, ... };
Physics::resolveBlockCollisionFromTop(nextFrame, blocks);
EXPECT_FALSE(nextFrame.onGround);       // 再着地しない
```

### テスト作成中のミス

#### BackgroundId uint8_tのオーバーフロー問題

「不明なBackgroundIdを渡すとstd::runtime_errorがthrowされるか」を検証するテストで，初期実装が正しく機能しない問題を発見した：

```cpp
// 初期実装（誤り）
const auto invalidId = static_cast<BackgroundId>(256);
```

BackgroundIdはenum class BackgroundId : std::uint8_tと定義されており，uint8_tの最大値は255である．
256をキャストすると**0にアップアラウンド**される．これによって，BackgroundId::Forest(0)と等しくなり，結果としてswitch文にヒットし，throwされないままテストが通過しなかった：

```text
static_cast<BackgroundId>(256) → uint8_t truncate → 0 == BackgroundId::Forest → switch 0 → throw文に到達しない
```

有効値が0〜4であるため，uint8_t範囲内かつ未定義の値(99)へ修正した：
...
week28はゲームの機能追加ではなく，これまでの実装したシステムの品質確認に注力した週となった．
GoogleTestを用いた単体テストの整備によって，GameUtilなど根幹をなすロジック層の動作仕様がコードとして確認できた．
week28をもって，手動プレイテストへの移行と技術・設計解説記事の執筆に着手する予定である．
