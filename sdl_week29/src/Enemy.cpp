...
/**
 * @brief Construct a new Enemy:: Enemy object(protected)
...
/**
 * @brief XXX <- Enemyにおいて，XXXのEnemyStateをリセットする関数(protected)
 * 
 */
void Enemy::resetEnemyStateForSpawn(){
    state = EnemyState::Alive;
    dyingTime = 0.0;
    onGround = false;
    hv = 0.0;
...
SDL_Rect Enemy::getCollisionRect() const{
    ...
    // return {static_cast<int>(x)+20, static_cast<int>(y)+20, sprite.getDrawWidth()-20, sprite.getDrawHeight()-20};
}
