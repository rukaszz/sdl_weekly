...
/**
 * @brief 天井との接触管理用のクランプ関数
 *         double blockRight = b.x + b.w;

 * if(vcs.vv >= 0.0)は重力加算後に呼ばれる前提の処理なので留意すること
 * vv < 0のときのみ上向き衝突判定が行われる(下降中は何もしない)
 * 
 * @param vcs: prevFeet/newFeet/x/width/vv/onGround/ignoreDropThrough
 *     @param prevTop: 1フレーム前のオブジェクトhead
 *     @param newTop: 更新時のオブジェクトのhead 
 * @param blocks: 描画しているブロックの情報 
 */ 
void Physics::resolveBlockCollisionFromBottom(VerticalCollisionState& vcs, const std::vector<Block>& blocks){
    // 上昇中のみ天井判定
    if(vcs.vv >= 0.0){
        return;
    }
    ...
    for(const auto& b : blocks){
        ...
        bool verticallyOverlaps = (prevTop     >= blockBottom) &&
                                  (blockBottom >= newTop);
        // 下からの接触で垂直/水平方向の重なりがあればブロック下部に対して補正する
 ...
 * 
 * @param vcs: prevFeet/newFeet/x/width/vv/onGround/ignoreDropThrough
 *     @param prevFeet: 1フレーム前のオブジェクトFeet
 *     @param newFeet: 更新時のオブジェクトFeet 
 * @param blocks: 描画しているブロックの情報 
 */ 
void Physics::resolveBlockCollisionFromTop(VerticalCollisionState& vcs, const std::vector<Block>& blocks){
    if(vcs.vv <= 0.0){
        vcs.onGround = false;
        return; // 上昇中 は何もしない
    }
